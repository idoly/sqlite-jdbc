...
# Keep only portable layouts and downcalls; callback upcalls use the native-image-safe hand-written shim.
	perl -ni -e 'print unless /\bC_(?:BOOL|SHORT|FLOAT|LONG)\s*=/' \
	    "$(JEXTRACT_STAGE)/$(JEXTRACT_PACKAGE_PATH)/SQLiteNative\$$shared.java"
	perl -ni -e 'print unless /\bLIBRARY_ARENA\b/' \
	    "$(JEXTRACT_STAGE)/$(JEXTRACT_PACKAGE_PATH)/SQLiteNative.java"
	find "$(JEXTRACT_STAGE)/$(JEXTRACT_PACKAGE_PATH)" \
	    -type f -name 'sqlite3_*.java' -delete
	...
	@header_sha="$$(sha256sum '$(SQLITE_SOURCE)/sqlite3.h' | awk '{print $$1}')"; \
	    printf '%s\n' \
	        ...
	        'jextract.postprocess=prune-unused-platform-scaffolding' \
